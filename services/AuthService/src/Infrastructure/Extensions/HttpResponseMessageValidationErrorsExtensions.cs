...
{
    internal static class HttpResponseMessageValidationErrorsExtensions
    {
        public static async Task<Result<TResponse>> HandleValidationErrorsAsync<TResponse>(this HttpResponseMessage responseMessage, CancellationToken ct)
        {
            ...
            if (errorResponse?.Reason == null || !errorResponse.Reason.Any())
                return Result.Fail<TResponse>("Response reasons content empty");
